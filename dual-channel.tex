...
\end{equation}
\par 此时的厂商的效益最大化，可看作厂商内部的最优化问题，因为市场价格与自身生产的关系已知，将式（\ref{p_star}）带入式（\ref{U_m}）得到到厂商效益：
\begin{equation}
...
\begin{equation}\label{t_star}
    \theta^*=\frac{\beta p-\alpha}{\lambda_r}
\end{equation}
\begin{equation}\label{w_star}
    w^*=\frac{\beta\eta-\lambda_r\lambda_m}{\lambda_r^2}p-\frac{\alpha\eta-\lambda_r\lambda_m c}{\lambda_r^2}-c
\end{equation}
\par 此时的经销商的效益最大化，可看作其自身的最优化问题，将式（\ref{t_star}）（\ref{w_star}）带入式（\ref{U_r}）得到到经销商效益：
\section{稳定性的数值分析}
