...
\begin{itemize}
    ...
    \item 在数值计算方面的工作，我们不再依赖常用的matlab工具包。我们编写了针对三个博弈目标（可非常容易地扩展到多个）的动力系统混沌分析python工具包，计算Hopf分岔轨迹以及模拟计算Lyapunov指数。基于混沌分析模拟的可并发性，我们主要支持基于cuda计算库和Nvidia GPU的高速计算。实验在GeForce 980Ti的环境下，完成了每秒约$2\times10^8$个样本，每个样本1000次的迭代计算。
\end{itemize}
\section{问题描述与模型建立}
...
\par \textbf{基本假设一：}我们考虑线性的供需关系，其中固有需求$\alpha$边际需求$\beta$，并且假设直销价格和经销商售价一致为$p$。在绿色供应链中，假设只存在一种绿色商品，其绿色程度$\theta$系数直接影响受到顾客额外的欢迎程度和生产成本。从而，该统一价格的双方市场需求分别为：
\begin{equation}
...
\par \textbf{基本假设二：}我们考虑政府对拥有不同$\theta$的商品进行价格补贴。由于方程的线性性质，该影响可以被看作是在消费者角度，通过价格优势形成的额外需求，从而可被统一归因为系数$\lambda$的影响。
\par \textbf{基本假设三：}我们假设制造商制造绿色商品的额外成本与$\frac{\theta^2}{2}$相关，系数$\eta$。制造商批发给零售商的价格$w$低于售价$p$，高于其基础成本$c$。则二者的收益（效用）可被表示为：
\begin{equation}
    \pi_m=(w-c)(\alpha-\beta p+\lambda_r\theta)+(p-c)(\alpha-\beta p+\lambda_m\theta)-\eta\frac{\theta^2}{2}
\end{equation}
\begin{equation}
    \pi_r=(p-w)(\alpha-\beta p+\lambda_r\theta)
\end{equation}
\par \textbf{基本假设四：}基于Stackelberg博弈模型，该供应链的批发价格$w$以及商品的绿色程度$\theta$先由供应商确定。其后，零售商通过调整销售价格来达到自己利益的最大化，此时基于假设一，直销渠道价格必须跟随。同时，政府可以通过调控价格补贴参数$\lambda$和成本系数$\eta$来调控市场。
\section{均衡求解}
\par 在基于Stackelberg假设下，供应商首先做出反应，供应商可以同时调整批发价格$w$和绿色程度$\theta$来使得自己的利益最大化。
\section{稳定性的数值分析}
\par
\section{结论}
\par
