...
\par \textbf{基本假设二：}我们考虑政府对拥有不同$\theta$的商品进行价格补贴$\tau$。由于方程的线性性质，该影响可以被看作是在消费者角度，通过价格优势形成的额外需求，从而可被统一归因为系数$\lambda$的影响，即：
\begin{equation}
...
\begin{equation}
    U_m^*(w,\theta)=\frac{1}{2}(w-c)(\alpha+\lambda_r\theta-\beta w)+\frac{1}{2}(\alpha+2\lambda_m\theta-\lambda_r\theta-\beta w)(\frac{\alpha+\lambda_r\theta+\beta w}{2\beta})-\eta\frac{\theta^2}{2}
\end{equation}
