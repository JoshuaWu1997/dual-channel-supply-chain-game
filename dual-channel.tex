...
\begin{equation}
    ...
    \right]
\end{equation}
\par 其中$h_1=\alpha-p^*+\lambda_r\theta^*$，$h_2=-\alpha-\lambda_r\theta^*-w^*$，$h_3=(1+\frac{1}{2\epsilon})\lambda_rw^*+(\frac{\lambda_r^*}{2\epsilon}-2\eta)\theta^*+(\frac{\alpha-2c}{2\epsilon}-c)\lambda_r$。对于该矩阵的特征值分析过于复杂，我们依然沿用前人的计算经验，套用了Jury判别条件。则该系统在Nash均衡点稳定的充分必要条件为：
